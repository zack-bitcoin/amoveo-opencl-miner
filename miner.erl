-module(miner)
.
-export([start/0, unpack_mining_data/1, speed_test/0, read_nonce/1]).
-define(Peer, "http://localhost:3011/").%for a test-node
...
start_many(N, Me) -> 
    Pid = spawn(fun() -> Me ! os:cmd("./amoveo_miner " ++ integer_to_list(N)) end),
    [Pid|start_many(N-1, Me)].
kill_os_mains() -> os:cmd("killall amoveo_miner").
